...
def profile(request):
    ...
    if request.method=='POST':
        ...
        if 'Password' in request.POST:
            ...
            new_password2 = request.POST.get('new2')
            
            if request.user.check_password(old_password):
                if new_password == new_password2:
                    request.user.set_password(new_password)
                    request.user.save()
                    update_session_auth_hash(request, request.user)  # Update the user's session
...
def setting(request):
    ...
    return render(request,'settings.html',context={'users':payment,'user':user})
...
def get_rows_back(soup):
    ...
    back_result=back_head[1].replace(' ','_')
    back_profit=back_head[2].replace(' ','_')
    back_r_f=back_head[5].replace(' ','_')
    back_e_dd_per=back_head[8].replace(' ','_').replace('%','')
    back_trades=back_head[9].replace(' ','_')
    for dt in soup['data']:
        data[dt['Pass']]={
            f"back_pass":dt['Pass'],
            f"back_{back_result}":dt['Result'],
...
def get_rows_forwad(soup,res,diff,file_name,balance,drawdown):
    ...
    for dt in soup['data']:
        ...
        if dt['Pass'].strip() in res :#and dt[0].text.strip() =='7703':
            ...
            res[Mypass][f"forward_{forward_result}"]=dt['Forward Result']
            ...
            try:
                ...
                res[Mypass][f"max_original_dd"]=round(max(float(res[Mypass]['back_Equity_DD_d']), float(dt['Equity DD %']))*(balance/100),2)
            ...
            try:
                res[Mypass][f"Lot_Multiple"]=round(drawdown/res[Mypass][f"max_original_dd"],2)
            ...
            try:
                res[Mypass][f"Estimated_Profit"]=round((res[Mypass][f"total_profit"]*res[Mypass][f"Lot_Multiple"]),2)
                
            except:
                res[Mypass][f"Estimated_Profit"]=0
            try:
                res[Mypass][f"Max_DD"]=round(res[Mypass][f"Lot_Multiple"]*res[Mypass][f"max_original_dd"],2)
